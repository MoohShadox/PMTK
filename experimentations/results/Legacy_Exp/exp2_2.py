...
EXP_TITLE = f"THETA_ELICITATION_{n_items}_{n_test_subsets}_3"
...
for t in range(10, 15, 3):
  for alpha in np.linspace(0.1, 0.4, 5):
    print("%%%% n_thetas: ",int((2**n_items - n_items)*alpha), "%%%%")
    for p in np.linspace(0.1, 0.8, 5):
      for reps in range(5):
        RTD = Random_Tierlist_Decider(items, p = p, n_theta= int((2**n_items - n_items)*alpha)+1, n_tiers = t)
